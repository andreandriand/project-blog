...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <form action="{{ route('author.media.index') }}" method="GET" class="flex flex-1 gap-3">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="{{ __('Cari file...') }}" class="input-field flex-1">
        </form>
        <form action="{{ route('author.media.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            @foreach($media as $item)
                <div x-data="{ showDetail: false }" class="group relative">
                    <div @click="showDetail = true" class="cursor-pointer bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 overflow-hidden hover:border-emerald-300 dark:hover:border-emerald-600 transition-colors">
                                ...
                                <div class="md:w-1/2 p-6">
                                    ...
                                    <div class="mt-4">
                                        ...
                                        <div class="flex gap-1">
                                            <input type="text" value="{{ $item->url }}" readonly class="input-field text-xs flex-1" id="url-author-{{ $item->id }}">
                                            <button onclick="navigator.clipboard.writeText(document.getElementById('url-author-{{ $item->id }}').value)" class="btn-secondary text-xs px-2">{{ __('Copy') }}</button>
                                        </div>
                                    </div>
                                    <div class="mt-4 flex gap-2">
                                        <form action="{{ route('author.media.destroy', $item) }}" method="POST" onsubmit="return confirm('{{ __('Yakin ingin menghapus?') }}')">
                                            @csrf @method('DELETE')
                                            <button class="inline-flex items-center px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-medium rounded-lg transition-colors">{{ __('Hapus') }}</button>
                                        </form>
