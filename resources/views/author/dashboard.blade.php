...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Total Artikel') }}</p>
                    <p class="text-3xl font-bold text-gray-900 dark:text-white mt-1">{{ number_format($stats['total_posts']) }}</p>
                    <p class="text-xs text-green-600 dark:text-green-400 mt-1">{{ $stats['published_posts'] }} {{ __('dipublikasi') }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-yellow-600 dark:text-yellow-400 mt-1">{{ __('menunggu persetujuan') }}</p>
                </div>
    ...
    @if($stats['rejected_posts'] > 0)
        <div class="mb-6 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 px-4 py-3 rounded-xl flex items-center">
            ...
            {{ __('Anda memiliki :count artikel yang ditolak.', ['count' => $stats['rejected_posts']]) }}
            <a href="{{ route('author.posts.index', ['status' => 'rejected']) }}" class="ml-1 underline font-medium">{{ __('Lihat & perbaiki') }}</a>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700">
            <div class="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('Artikel Terbaru') }}</h2>
                <a href="{{ route('author.posts.index') }}" class="text-sm text-emerald-600 dark:text-emerald-400 hover:underline">{{ __('Lihat Semua') }}</a>
            </div>
            <div class="divide-y divide-gray-100 dark:divide-gray-700">
                @forelse($recentPosts as $post)
                    <div class="p-4 flex items-center justify-between">
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ $post->category->name ?? __('Tanpa Kategori') }} &middot; {{ $post->created_at->diffForHumans() }}</p>
                        </div>
                        @php
                ...
                @empty
                    <div class="p-6 text-center text-gray-500 dark:text-gray-400 text-sm">{{ __('Belum ada artikel') }}</div>
                @endforelse
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700">
            <div class="p-6 border-b border-gray-100 dark:border-gray-700">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('Aksi Cepat') }}</h2>
            </div>
            <div class="p-6 space-y-3">
                <a href="{{ route('author.posts.create') }}" class="flex items-center p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl hover:bg-emerald-100 dark:hover:bg-emerald-900/30 transition-colors">
                    ...
                    <div>
                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ __('Tulis Artikel Baru') }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('Mulai menulis artikel baru') }}</p>
                    </div>
                </a>
                <a href="{{ route('author.comments.index') }}" class="flex items-center p-4 bg-purple-50 dark:bg-purple-900/20 rounded-xl hover:bg-purple-100 dark:hover:bg-purple-900/30 transition-colors">
                    ...
                    <div>
                        <p class="text-sm font-medium text-gray-900 dark:text-white">{{ __('Kelola Komentar') }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('Lihat dan kelola komentar di artikel Anda') }}</p>
                    </div>
                </a>
            </div>
        </div>
