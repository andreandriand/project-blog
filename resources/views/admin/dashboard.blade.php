...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Total Komentar') }}</p>
                    <p class="text-3xl font-bold text-gray-900 dark:text-white mt-1">{{ number_format($stats['total_comments']) }}</p>
                    <p class="text-xs text-yellow-600 dark:text-yellow-400 mt-1">{{ $stats['pending_comments'] }} {{ __('menunggu') }}</p>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700">
            <div class="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700">
                ...
                <a href="{{ route('admin.posts.index') }}" class="text-sm text-primary-600 dark:text-primary-400 hover:underline">{{ __('Lihat Semua') }}</a>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700">
            <div class="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-700">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('Komentar Terbaru') }}</h2>
                <a href="{{ route('admin.comments.index') }}" class="text-sm text-primary-600 dark:text-primary-400 hover:underline">{{ __('Lihat Semua') }}</a>
            </div>
            <div class="divide-y divide-gray-100 dark:divide-gray-700">
                @forelse($recentComments as $comment)
                    <div class="p-4">
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <span class="badge {{ $comment->is_approved ? 'badge-success' : 'badge-warning' }}">{{ $comment->is_approved ? __('Disetujui') : __('Pending') }}</span>
                        </div>
                ...
                @empty
                    <div class="p-6 text-center text-gray-500 dark:text-gray-400 text-sm">{{ __('Belum ada komentar') }}</div>
                @endforelse
            </div>
        </div>
    </div>
