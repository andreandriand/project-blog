...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <p class="text-gray-500 dark:text-gray-400 text-sm">{{ $tags->total() }} tag</p>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                @forelse($tags as $tag)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                        ...
                        <td class="px-6 py-4"><span class="badge badge-primary">{{ $tag->posts_count }}</span></td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end space-x-2">
                                ...
                                <form action="{{ route('admin.tags.destroy', $tag) }}" method="POST" onsubmit="return confirm('Yakin?')">@csrf @method('DELETE')
                ...
                @empty
                    <tr><td colspan="4" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">Belum ada tag</td></tr>
                @endforelse
            </tbody>
        </table>
        @if($tags->hasPages()) <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-700">{{ $tags->links() }}</div> @endif
    </div>
@endsection
